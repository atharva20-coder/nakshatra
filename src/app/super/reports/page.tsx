...
import { getAssignmentReportAction } from "@/actions/reports.action";
import { AssignmentReportClient } from "@/components/reports/assignment-report-client";
import { Card, CardContent } from "@/components/ui/card";

interface PageProps {
  searchParams: Promise<{ [key: string]: string | string[] | undefined }>; // FIX: searchParams is a Promise
}
...
export default async function AssignmentReportsPage({ searchParams }: PageProps) {
  ...
  // 2. Use the resolved searchParams
  ...
  // 3. Fetch report data
  const { auditAssignments, cmAssignments, error } = await getAssignmentReportAction({ month, year });
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-400 mt-1">
              View all agency assignments created in a specific month and year.
            </p>
          </div>
          <ReturnButton href="/super/dashboard" label="Back to Dashboard" />
        </div>
