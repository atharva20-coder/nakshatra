// src/components/super-admin/ScheduledAnnouncementManager.tsx
"use client";
...
import { Loader2, Trash2, Megaphone, Send, Calendar, Clock, CheckCircle, PlayCircle } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { useRouter } from "next/navigation";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";

type GetAnnouncementsReturn = Awaited<ReturnType<typeof getAllAnnouncementsForSuperAdminAction>>;
type SuccessAnnouncementsReturn = Extract<GetAnnouncementsReturn, { success: true }>;
type AnnouncementWithDetails = SuccessAnnouncementsReturn["data"] extends (infer U)[] ? U : never;
...
export function ScheduledAnnouncementManager({ initialAnnouncements }: ScheduledAnnouncementManagerProps) {
  ...
  const handleCreate = () => {
    ...
    startTransition(async () => {
      ...
      if (!result.success) {
        ...
        toast.success(isScheduled ? "Announcement scheduled successfully!" : "Announcement published successfully!");
        setTitle("");
        setContent("");
        setAudience([]);
  ...
  const scheduledAnnouncements = announcements.filter(a => !a.isPublished && a.isScheduled);
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <Card className="lg:col-span-2">
        <CardHeader>
          ...
          <CardDescription>Manage published and scheduled announcements</CardDescription>
        </CardHeader>
        <CardContent>
          <Tabs defaultValue="published">
            ...
            <TabsContent value="published" className="space-y-4 mt-4">
              {publishedAnnouncements.length === 0 ? (
                <p className="text-center text-gray-500 py-8">No published announcements.</p>
              ) : (
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-500 mb-3">
                          Published: {ann.publishedAt ? new Date(ann.publishedAt).toLocaleString() : 'N/A'}
                        </p>
            ...
            <TabsContent value="scheduled" className="space-y-4 mt-4">
              {scheduledAnnouncements.length === 0 ? (
                <p className="text-center text-gray-500 py-8">No scheduled announcements.</p>
              ) : (
                scheduledAnnouncements.map((ann) => (
                  <div key={ann.id} className="border-2 border-blue-200 p-4 rounded-lg bg-blue-50 dark:bg-blue-900/20">
                    <div className="flex justify-between items-start gap-4">
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-blue-700 mb-3 flex items-center gap-1">
                          <Calendar className="h-3 w-3" />
                          Will publish: {ann.scheduledFor ? new Date(ann.scheduledFor).toLocaleString() : 'N/A'}
                        </p>
                        <p className="text-sm mb-3">{ann.content}</p>
                        <div className="flex flex-wrap gap-2">
                          {ann.audience.map((role) => (
